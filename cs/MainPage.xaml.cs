...
namespace FaceDetection
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private bool _detectionFlag = false;
        ...
        {
            _detectionFlag = !_detectionFlag;
            while (_detectionFlag)
            {
                ...
                using (var randomAccessStream = await file.OpenAsync(FileAccessMode.Read))
                {
                    ...
                    HttpStreamContent streamContent = new HttpStreamContent(randomAccessStream);

                    //send request
                    var myFilter = new Windows.Web.Http.Filters.HttpBaseProtocolFilter();
                    myFilter.AllowUI = false;
                    var client = new Windows.Web.Http.HttpClient(myFilter);
                    Windows.Web.Http.HttpResponseMessage result = await client.PostAsync(new Uri("http://127.0.0.1/image"), streamContent);
                    string stringReadResult = await result.Content.ReadAsStringAsync();

                    InfoText.Text = stringReadResult;
                }
            }
        }
        ...
        private async Task CleanupCameraAsync()
        {
            ...
            if (_isInitialized)
            {
                ...
                if (_detectionFlag == true)
                {
                    _detectionFlag = false;
                }
